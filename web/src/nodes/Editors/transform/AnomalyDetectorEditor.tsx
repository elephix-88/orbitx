...
const AnomalyDetectorEditor: React.FC<AnomalyDetectorEditorProps> = ({
 ...
 <div className="flex items-center gap-2 p-3 rounded-lg bg-bg-card border border-line-1 text-xs text-text-3">
 ...
 <label className="text-xs font-medium text-text-2">Metrics to Monitor</label>
 ...
 : 'bg-bg-card border-line-1 text-text-3 hover:text-text-2'
 )}
 >
 {column}
 </button>
 ...
 {metrics.length === 0 && (
 <p className="text-[10px] text-text-3">Select columns to monitor for anomalies.</p>
 )}
 </div>
 )}
 ...
 <label className="text-xs font-medium text-text-2">Group By</label>
 <select
 value={groupBy}
 ...
 <label className="text-xs font-medium text-text-2">Detection Settings</label>

 <div className="grid grid-cols-2 gap-3">
 {/* Window days */}
 <div className="flex flex-col gap-1">
 <label className="text-[10px] text-text-3">Baseline Window (days)</label>
 <input
 type="number"
 min={3}
 max={30}
 ...
 {/* Threshold */}
 <div className="flex flex-col gap-1">
 <label className="text-[10px] text-text-3">Threshold (%)</label>
 <input
 type="number"
 min={1}
 max={500}
 ...
 {/* Max alerts per day */}
 <div className="flex flex-col gap-1">
 <label className="text-[10px] text-text-3">Max Alerts per Day (per group)</label>
 <input
 type="number"
 min={1}
 max={100}
 ...
 {metrics.length > 0 && (
 <div className="flex flex-col gap-1.5">
 <label className="text-[10px] text-text-3">Output Columns Added</label>
 <div className="flex flex-wrap gap-1">
 {metrics.map((metric) => (
 <span
 key={metric}
 className="px-1.5 py-0.5 text-[10px] rounded bg-bg-card border border-line-1 text-text-3"
 >
 {metric}_is_anomaly
 </span>
 ))}
