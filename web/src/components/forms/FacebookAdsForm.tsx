...
export const FacebookAdsForm: React.FC<FacebookAdsFormProps> = ({
 ...
 <label className="text-xs font-medium text-text-2">Facebook Connection</label>
 <a
 href="/connections"
 ...
 conn.id === formData.connection_id && "bg-blue-soft"
 )}
 >
 ...
 <div className="text-sm text-text-1 truncate">{account.name}</div>
 </div>
 <input
 type="checkbox"
 className="hidden"
 ...
 className={cn(
 "w-full px-3 py-2.5 text-left text-sm flex items-center justify-between hover:bg-bg-card transition-colors",
 preset.value === formData.time_config?.time_preset && "bg-blue-soft"
 )}
 >
