...
const StatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
 ...
 case 'SUCCESS':
 return (
 <span className="flex items-center gap-1 text-[10px] font-semibold text-success bg-success-bg border border-success/20 rounded-md px-1.5 py-0.5">
 <CheckCircle2 size={9} />
 OK
 </span>
 );
 case 'FAILED':
 return (
 <span className="flex items-center gap-1 text-[10px] font-semibold text-error bg-danger-bg border border-danger-border/20 rounded-md px-1.5 py-0.5">
 <XCircle size={9} />
 FAILED
 </span>
 );
 case 'RUNNING':
 return (
 <span className="flex items-center gap-1 text-[10px] font-semibold text-warning bg-warning-bg border border-warning/20 rounded-md px-1.5 py-0.5">
 ...
 case 'PENDING':
 return (
 <span className="flex items-center gap-1 text-[10px] font-semibold text-text-3 bg-bg-muted border border-line-1 rounded-md px-1.5 py-0.5">
 <Clock size={9} />
 PENDING
 </span>
 );
...
export const ExecutionHistoryPanel: React.FC<ExecutionHistoryPanelProps> = ({
 ...
 >
 {/* Header */}
 <div className="flex items-center justify-between px-4 py-3 border-b border-line-1 flex-shrink-0">
 <div className="flex items-center gap-2">
 <History className="w-4 h-4 text-blue-primary" />
 <span className="text-sm font-semibold text-text-1">Execution History</span>
 </div>
 <div className="flex items-center gap-1">
 <button
 onClick={fetchList}
 disabled={loading}
 className="p-1.5 rounded-lg text-text-3 hover:text-text-1 hover:bg-bg-card transition-colors disabled:opacity-40"
 title="Refresh list"
 >
 ...
 <button
 onClick={onClose}
 className="p-1.5 rounded-lg text-text-3 hover:text-text-1 hover:bg-bg-card transition-colors"
 title="Close"
 >
 <X size={13} />
 </button>
 ...
 <p className="text-xs text-text-2">{error}</p>
 <button
 onClick={fetchList}
 className="text-xs text-blue-primary hover:text-blue-primary underline"
 >
 Retry
 </button>
 </div>
 ...
 <p className="text-sm text-text-2">No executions yet</p>
 ...
 'hover:bg-bg-card',
 isActive && 'bg-blue-soft border-l-2 border-blue-primary'
 )}
 >
 {/* Status icon */}
 ...
 <ChevronRight size={12} className="text-text-3 flex-shrink-0 mt-1" />
 </button>
