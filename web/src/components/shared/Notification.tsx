...
const notificationVariants = cva(
 'pointer-events-auto relative flex items-center justify-between space-x-4 overflow-hidden rounded-lg p-5 shadow-sm transition-all duration-300 bg-bg-card border border-line-1',
 {
 variants: {
 type: {
...
const NotificationItem = ({
 ...
 <p className="text-base font-semibold text-text-1 truncate" title={title}>
 {title}
 </p>
 {message && (
 <p className="mt-1 text-sm text-text-2 break-words" title={message}>
 {message}
 </p>
 )}
 {Array.isArray(details) && details.length > 0 && (
 <ul className="mt-2 space-y-1 text-sm text-text-2 list-disc list-inside">
 {details.map((d, i) => (
 ...
 <button
 type="button"
 className="inline-flex rounded-md p-1 text-text-3 hover:text-text-2 hover:bg-bg-muted focus:outline-none focus:ring-2 focus:ring-blue-primary transition-colors"
 onClick={onClose}
