...
const BaseEditorWrapper = ({
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-black/50 backdrop-blur-sm animate-fade-in">
			<form
				onSubmit={onSubmit}
				className="w-full max-w-5xl h-[85vh] sm:h-[80vh] flex flex-col bg-bg-page rounded-2xl shadow-2xl border border-line-1 animate-scale-in"
			>
				<div className="flex-shrink-0 flex justify-between items-center px-6 py-4 border-b border-line-1">
					<div className="flex items-center gap-4">
						<div className="w-10 h-10 rounded-lg flex items-center justify-center bg-bg-card">
							{icon || <Settings className="w-5 h-5 text-text-2" />}
						</div>
						<div>
					...
					<div className="flex items-center gap-2">
						{onDeleteNode && (
							<button
								...
								className="p-2 text-text-3 hover:text-error hover:bg-bg-card rounded-lg transition-colors"
								title="Delete node"
							>
								<Trash2 size={18} />
							</button>
						...
						<button
							...
							className="p-2 text-text-3 hover:text-text-1 hover:bg-bg-card rounded-lg transition-colors"
							title="Close"
						>
							<X size={18} />
						</button>
				...
				<div className="flex-shrink-0 border-t border-line-1 px-6 py-4">
					<Button
						type="submit"
			...
			<ConfirmDialog
				...
				message={
					<p>
						You have unsaved changes in this node. Closing will discard them.
					</p>
				}
				confirmText="Discard"
