...
const statusConfig: Record<string, StatusConfig> = {
 ...
 syncing: {
 label: 'Syncing',
 dotClass: 'bg-blue-primary animate-pulse',
...
export const ConnectionRow: React.FC<ConnectionRowProps> = ({
 ...
 <div
 className={cn(
 'group flex items-center gap-4 px-4 py-3 bg-bg-card/20 border border-line-1 rounded-lg transition-all',
 'hover:bg-bg-card/40 hover:border-line-1/80'
 )}
 >
 {/* Icon */}
 <div
 ...
 'group-hover:bg-blue-soft group-hover:text-blue-primary'
 )}
 >
 {icon}
 </div>
 ...
 {lastSynced && <span className="text-text-3/50">•</span>}
 </>
 )}
 {lastSynced && (
 ...
 <p className="text-xs text-text-3 mt-0.5 truncate">{description}</p>
 )}
 </div>
 ...
 'hover:bg-bg-card',
 menuOpen && 'bg-bg-card text-text-1'
 )}
 >
 ...
 {/* Dropdown Menu */}
 {menuOpen && (
 <div className="absolute right-0 top-full mt-1 w-40 py-1 bg-bg-page border border-line-1 rounded-lg shadow-lg z-50">
 {onViewDetails && (
 <button
 onClick={() => {
 setMenuOpen(false);
 ...
 className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-2 hover:text-text-1 hover:bg-bg-card transition-colors"
 >
 ...
 isConnecting
 ? 'bg-blue-primary text-white cursor-wait'
 : 'bg-blue-primary hover:bg-blue-primary-hover text-white shadow-sm hover:shadow-md hover:shadow-primary-500/20'
 )}
 >
 {isConnecting ? (
 <>
